...
if [ "${LAFDB_RUN_DRM_DAEMON:-0}" = "1" ]; then
  ...
  echo "LAFDB_RUN_DRM_DAEMON is not 1; starting web server only"
fi

exec /lafdb/bin/lafdb --cf-csp --duckdb-cache /lafdb/laftel/data.duckdb --duckdb-build-on-reload-only
