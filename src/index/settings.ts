...
export function initSettings({ onRefreshFeed }: InitSettingsOptions) {
  ...
  function simpleMarkdown(md: string): string {
    ...
    let res = md.replace(/```([\s\S]*?)```/g, (_, code) => {
      ...
      codes.push(
        `<pre style="font-family:monospace;white-space:pre-wrap;word-break:break-word;background:#000;padding:12px;border-radius:8px;border:1px solid #222;overflow-x:auto;margin:1em 0;font-size:12px;color:#ccc;line-height:1.4">${escapeHtml(code.trim())}</pre>`,
      );
      return `\n\n${id}\n\n`;
    });
  ...
  function isSafariLikeAppleDevice(): boolean {
    ...
    const isiPhoneOrIPad = /\b(iPhone|iPad|iPod)\b/i.test(ua) || (platform === "MacIntel" && touchPoints > 1);
    const isSafari =
      /Apple/i.test(vendor) &&
      /Safari/i.test(ua) &&
  ...
  function getBufferPruneDelayPref(): number {
    return clampBufferPruneDelay(
      parseInt(localStorage.getItem("player_buffer_prune_delay") || String(DEFAULT_BUFFER_PRUNE_DELAY), 10),
    );
  }
  ...
  function updateBufferUI(ahead: number, behind: number, pruneDelay: number) {
    ...
    playerAdvBtn?.classList.toggle(
      "active",
      ahead !== DEFAULT_BUFFER_AHEAD || behind !== DEFAULT_BUFFER_BEHIND || pruneDelay !== DEFAULT_BUFFER_PRUNE_DELAY,
    );
  }
  ...
  btnSyncEpisodes?.addEventListener("click", () => {
    ...
    if (m.itemCount === 0) {
      if (
        confirm(
          "에피소드 데이터를 받으려면 먼저 작품 데이터가 필요합니다. 작품과 에피소드를 모두 다운로드하시겠습니까?",
        )
      ) {
  ...
  document.getElementById("btn-clear-all")?.addEventListener("click", async () => {
    if (
      !confirm(
        "모든 저장 데이터(시청 기록, 설정, 로컬 DB, 캐시, 서비스 워커)를 삭제할까요?\n이 작업은 되돌릴 수 없습니다.\n\n삭제 후에도 업데이트가 안 된다면 Ctrl+Shift+R, 모바일에서는 다른 브라우저 앱으로 재접속해 주세요.",
      )
    )
      return;
