...
async function appendOfflineEpisodes(
	episodes: OfflineEpisodeRecord[],
	itemCount: number,
	scope: OfflineScope,
) {
...
async function beginOfflineEpisodeStage(itemCount: number, scope: OfflineScope, nextItemIndex: number, totalItems: number) {
	const db = await openOfflineDb();
	return await new Promise((resolve, reject) => {
		const tx = db.transaction([OFFLINE_EPISODE_STORE, OFFLINE_META_STORE], "readwrite");
...
async function refreshOfflineMeta() {
	...
	offlineMeta = {
		...
		scopes: {
			items: meta?.scopes && typeof meta.scopes === "object"
				? (meta.scopes as Record<string, unknown>).items !== false
				: true,
			episodes: meta?.scopes && typeof meta.scopes === "object"
				? (meta.scopes as Record<string, unknown>).episodes === true
				: false,
		},
		syncState: {
			inProgress: meta?.syncState && typeof meta.syncState === "object"
				? (meta.syncState as Record<string, unknown>).inProgress === true
				: false,
			stage: meta?.syncState && typeof meta.syncState === "object"
				? (String((meta.syncState as Record<string, unknown>).stage || "") as "items" | "episodes" | "")
				: "",
			nextItemIndex: meta?.syncState && typeof meta.syncState === "object"
				? Number((meta.syncState as Record<string, unknown>).nextItemIndex || 0)
				: 0,
			totalItems: meta?.syncState && typeof meta.syncState === "object"
...
function scoreOfflineMatch(item: OfflineItemRecord, query: string) {
	...
	const dist = Math.min(
		levenshtein(q, nameSlice),
		qDecomp ? levenshtein(qDecomp, decompSlice) : Infinity,
	);
...
async function queryOfflineItems(
	state: { q?: string; original?: string | null; ending?: string | null; medium?: string | null; genre?: string | null; year?: string | null; sort?: string },
	offset: number,
	size: number,
) {
	const allItems = await loadOfflineItems();
	...
	if (!q) {
		switch (state.sort) {
			case "avg_rating":
				...
				break;
			case "update":
				filtered.sort((a, b) => String(b.latest_episode_release_datetime || "").localeCompare(String(a.latest_episode_release_datetime || "")) || a._sortRecent - b._sortRecent);
				break;
			default:
