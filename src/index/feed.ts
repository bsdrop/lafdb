...
<div class="card-body">
	...
	async function fetchPage(reset = false) {
		...
		try {
			if (shouldPreferOfflineItemSearch()) {
				...
				offset += result.items.length;
				renderItems(result.items, isFirstPage);
				setFeedStatus(isFirstPage ? "" : `오프라인 메타데이터에서 ${offset.toLocaleString()}/${total.toLocaleString()}개 불러옴`);
				return;
			}
	...
	function syncChipsToState() {
		document.querySelectorAll<HTMLElement>(".chip[data-sort]").forEach((c) =>
			c.classList.toggle("active", c.dataset["sort"] === state.sort),
		);
		document.querySelectorAll<HTMLElement>(".chip[data-genre]").forEach((c) =>
			c.classList.toggle("active", c.dataset["genre"] === state.genre),
		);
		document.querySelectorAll<HTMLElement>(".chip[data-medium]").forEach((c) =>
			c.classList.toggle("active", c.dataset["medium"] === state.medium),
		);
		document.querySelectorAll<HTMLElement>(".chip[data-original]").forEach((c) =>
			c.classList.toggle("active", c.dataset["original"] === state.original),
		);
		document.querySelectorAll<HTMLElement>(".chip[data-ending]").forEach((c) =>
			c.classList.toggle("active", c.dataset["ending"] === state.ending),
		);
		document
