...
const Mp4Mux = (() => {
  ...
  function assemble(
    ...
    for (let i = 0; i < n; i++) {
      if (i < vSegs.length)
        parts.push(vOffset ? shiftTfdtBaseTime(keepMoofMdat(vSegs[i]), -vOffset) : keepMoofMdat(vSegs[i]));
      if (i < aSegs.length)
        parts.push(
          rewriteAudioTfhdTrackId(
            aOffset ? shiftTfdtBaseTime(keepMoofMdat(aSegs[i]), -aOffset) : keepMoofMdat(aSegs[i]),
            2,
          ),
        );
    }
...
function _dlInit(mpdUrl: string, workerOrPlayer: unknown, isWorker: boolean): void {
  _dlMpdUrl = mpdUrl;
  if (isWorker) {
    _dlWorker = workerOrPlayer as Worker;
    _dlPlayerRef = null;
  } else {
    _dlPlayerRef = workerOrPlayer as DlPlayerRef;
    _dlWorker = null;
  }
...
function _dlHandleMsg(data: Record<string, unknown>): void {
  ...
  else if (data["type"] === "dlTracksReady") cb({ tracks: data["tracks"], videoReps: data["videoReps"] });
}
...
async function _dlRun(repId: string, onProgress: (p: ProgressInfo) => void): Promise<Uint8Array> {
  ...
  async function fetchBuf(url: string): Promise<Uint8Array> {
    ...
    for (let attempt = 0; attempt <= FETCH_RETRY_LIMIT; attempt++) {
      ...
      try {
        ...
        console.warn(
          `[DL] partial/network transfer failed; retry ${attempt + 1}/${FETCH_RETRY_LIMIT} in ${delay}ms`,
          err,
        );
  ...
  if (window._dlDurSecs == null) {
    ...
    if (epId) {
      try {
        ...
        console.error("[PLAYER] running_time parse failed:", e);
      }
    }
  }
  ...
  const durSecs =
    window._dlDurSecs ?? (lastV.time + lastV.duration - (video.timeline![0]?.time ?? 0)) / video.timescale;
  ...
  const [vInit, aInit] = await Promise.all([_dlDecrypt(vInitRaw, true, "video"), _dlDecrypt(aInitRaw, true, "audio")]);
  ...
  const [vSegs, aSegs] = await Promise.all([
    downloadTrack(video, vCount, "video", (b) => {
      vDone++;
      _addBytes(b, "video");
      emitProgress();
    }),
    downloadTrack(audio, aCount, "audio", (b) => {
      aDone++;
      _addBytes(b, "audio");
      emitProgress();
    }),
